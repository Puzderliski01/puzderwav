...
@HiltAndroidApp
class PuzderWavApplication : Application(), ImageLoaderFactory {
    ...
    @Inject lateinit var streamExtractor: dagger.Lazy<com.puzderwav.app.data.music.YouTubeStreamExtractor>
    @Inject lateinit var ytMusicSyncManager: dagger.Lazy<com.puzderwav.app.data.ytmusic.YtMusicSyncManager>
    @Inject lateinit var ytMusicHistorySyncManager: dagger.Lazy<com.puzderwav.app.data.ytmusic.YtMusicHistorySyncManager>
    @Inject lateinit var likedSongsManager: dagger.Lazy<com.puzderwav.app.data.playlist.LikedSongsManager>
    @Inject lateinit var trackDownloadManager: dagger.Lazy<com.puzderwav.app.data.download.TrackDownloadManager>
    @Inject lateinit var appLocaleManager: dagger.Lazy<com.puzderwav.app.util.AppLocaleManager>
    ...
    override fun attachBaseContext(base: Context) {
        ...
        super.attachBaseContext(com.puzderwav.app.util.AppLocaleManager.wrap(base))
    ...
    override fun onCreate() {
        ...
        runCatching { com.puzderwav.app.playback.PlaybackDiagnostics.install(this) }
        runCatching { com.puzderwav.app.data.music.potoken.BotGuardTokenGenerator.initialize(this) }
        applicationScope.launch(Dispatchers.IO) {
            ...
            runCatching { likedSongsManager.get().start() }
                .onFailure { android.util.Log.e("PuzderWavStartup", "Liked Songs startup disabled", it) }
        }
        ...
        applicationScope.launch {
            ...
            runCatching { ytMusicSyncManager.get().start() }
                .onFailure { android.util.Log.e("PuzderWavStartup", "YT sync startup disabled", it) }
        }
        ...
        applicationScope.launch {
            ...
            runCatching { ytMusicHistorySyncManager.get().start() }
                .onFailure { android.util.Log.e("PuzderWavStartup", "YT history sync startup disabled", it) }
        }
        ...
        applicationScope.launch(Dispatchers.IO) {
            ...
            runCatching { trackDownloadManager.get().syncDownloadsFromStorage() }
                .onFailure { android.util.Log.e("PuzderWavStartup", "Download sync startup failed", it) }
        }
        // A widget is a separate RemoteViews surface, so it needs an explicit
        // refresh whenever PuzderWav's live theme changes. The widget's palette
        ...
        applicationScope.launch(Dispatchers.IO) {
            ...
            try {
                themeRepository.get().uiState
                    ...
                    .collect {
                        WidgetUpdater.refreshTheme(this@PuzderWavApplication)
                    }
            } catch (cancellation: CancellationException) {
                throw cancellation
            } catch (error: Throwable) {
                android.util.Log.e("PuzderWavStartup", "Widget theme observer disabled", error)
            }
        }
    }
    ...
    override fun newImageLoader(): ImageLoader {
        ...
        val imageClient = try {
            ...
            android.util.Log.e("PuzderWavStartup", "Shared artwork client unavailable; using isolated client", error)
            okhttp3.OkHttpClient.Builder().build()
        } catch (error: LinkageError) {
            android.util.Log.e("PuzderWavStartup", "Shared artwork client unsupported; using isolated client", error)
            okhttp3.OkHttpClient.Builder().build()
        }
