...
import com.puzderwav.app.MainActivity
...
import com.puzderwav.app.data.repository.ScrobbleRepository
import com.puzderwav.app.data.repository.ThemeRepository
import com.puzderwav.app.service.ScrobbleDebugLog
import com.puzderwav.app.widget.ActiveMediaSessionHolder
...
class MusicPlaybackService : MediaBrowserServiceCompat() {
    ...
    @Inject lateinit var artworkRepository: com.puzderwav.app.data.artwork.ArtworkRepository
    ...
    override fun onCreate() {
        ...
        playbackWifiLock = wifiManager?.createWifiLock(
            ...
            "PuzderWav:PlaybackWifiLock",
        )?.apply { setReferenceCounted(false) }

        createNotificationChannel()
        runCatching {
            val session = MediaSessionCompat(this, "PuzderWavPlayer").apply {
                setFlags(
        ...
        scope.launch {
            artworkRepository.resolved.collect { map ->
                ...
                val key = com.puzderwav.app.data.artwork.ArtworkNormalizer.cacheKey(current.title, current.artist)
    ...
    override fun onGetRoot(
        ...
        // (onConnectionFailed), which is why KWGT never listed PuzderWav as
    ...
    private fun promoteForPlayback(): Boolean {
        ...
        val promoted = runCatching {
            ...
            val minimal = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
             ...
             .setContentTitle(musicPlayer.state.value.current?.title ?: "PuzderWav")
    ...
    private fun cleanTrackMetadata(title: String, artist: String): CleanTrackMetadata {
        ...
        var cleanArtist = artist.trim()
            ...
            .trim()
        if (com.puzderwav.app.util.ArtistHelper.isPlayCountOrStat(cleanArtist)) {
            cleanArtist = ""
        }
    ...
    private fun requestArtwork(track: PlayableTrack?) {
        ...
        val key = com.puzderwav.app.data.artwork.ArtworkNormalizer.cacheKey(track.title, track.artist)
    ...
    private fun buildNotification(state: MusicPlayerState, art: Bitmap?): Notification {
        ...
        if (useSystemMediaStyle()) {
            ...
            return builder
                .setSmallIcon(R.drawable.ic_launcher_logo)
                .setContentTitle(state.current?.title ?: "PuzderWav")
    ...
    private fun notificationRemoteViews(
        ...
        setTextViewText(R.id.notification_title, state.current?.title ?: "PuzderWav")
        ...
        runCatching {
            ...
            nm.createNotificationChannel(
                NotificationChannel(CHANNEL_ID, "Music playback", NotificationManager.IMPORTANCE_DEFAULT).apply {
                    description = "Native PuzderWav playback controls"
                    setShowBadge(false)
                },
            )
        }.onFailure { error ->
    ...
    private companion object {
        ...
        const val CHANNEL_ID = "PuzderWav_playback_v2"
        const val LEGACY_CHANNEL_ID = "PuzderWav_playback"
        const val NOTIFICATION_ID = 4102
        const val ACTION_PREVIOUS = "com.puzderwav.app.playback.PREVIOUS"
        const val ACTION_TOGGLE = "com.puzderwav.app.playback.TOGGLE"
        const val ACTION_NEXT = "com.puzderwav.app.playback.NEXT"
        const val ACTION_SHUFFLE = "com.puzderwav.app.playback.SHUFFLE"
        const val ACTION_REPEAT = "com.puzderwav.app.playback.REPEAT"
        const val ACTION_STOP = "com.puzderwav.app.playback.STOP"
    }
}
