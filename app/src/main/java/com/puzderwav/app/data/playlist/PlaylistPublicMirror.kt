...
private const val MIRROR_TYPE = "PuzderWav-playlists"
...
/** Public Downloads/PuzderWav JSON mirror. Room remains the fast runtime
...
class PlaylistPublicMirror @Inject constructor(
    ...
    suspend fun importAndMerge(content: String): Result<Int> = mutex.withLock {
        val mirror = decode(content)
            ?: return@withLock Result.failure(IllegalArgumentException("Invalid PuzderWav playlist JSON"))
        try {
            dao.upsertAll(mirror.playlists.map { it.toEntity() })
